...
const LikedSongs = () => {
    ...
    obj.userId = user._id
    console.log(obj);
    axios.post("http://localhost:3001/user/getlikedsongs", obj)
    .then(res => {
        ...
        <div className='md:h-full items-center text-gray-600'>
            <div className='container flex flex-col px-5 py-24 mx-auto'>
                <div className='text-center mb-12'>
                    <h5 className='text-base md:text-lg text-[#EA0C5C] mb-1'>Songs That You Love </h5>
                    <h1 className='text-4xl md:text-6xl text-white font-semibold'>Liked Songs</h1>
                </div>
            </div>
            ...
            {songData.length>0 ?
                <div className='flex flex-wrap p-6 items-centerz justify-center md:grid grid-cols-4 gap-4 items-stretch'>
                    {songData.map((song) => (
                    <div className="flex justify-center m-4 w-[80%] h-full z-[1]">
                    ...
                    <Link to="/song" state={song.id} data-mdb-ripple="true" data-mdb-ripple-color="light">
                        ...
                        <div className="p-6">
                        <h5 className="text-xl font-medium mb-2 text-white">{song.name}</h5>
